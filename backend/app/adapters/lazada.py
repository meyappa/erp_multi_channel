...
class LazadaAdapter(MarketplaceAdapter):
    ...
    async def push_inventory(
        ...
        return PushResult(ok=True, external_id=f"lzd-{sku}", message=f"Lazada qty set to {quantity}")

    async def fetch_orders(
        self, credentials: dict[str, str], since: Optional[datetime] = None
    ) -> list[RemoteOrder]:
        _ = credentials, since
        return orders_for(self.code, "lzd")
    ...
    async def push_listing(self, credentials: dict[str, str], listing: dict[str, Any]) -> PushResult:
        _ = credentials
        return PushResult(ok=True, external_id=f"lzd-{listing.get('sku')}", message="Listing upserted on Lazada")

    async def fetch_payouts(
        self, credentials: dict[str, str], since: Optional[datetime] = None
    ) -> list[RemotePayout]:
        _ = credentials, since
        orders = orders_for(self.code, "lzd")
        return payouts_for(self.code, "lzd", orders)
