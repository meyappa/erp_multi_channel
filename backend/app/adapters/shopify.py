...
class ShopifyAdapter(MarketplaceAdapter):
    ...
    async def push_inventory(
        ...
        return PushResult(ok=True, external_id=f"shy-{sku}", message=f"Shopify qty set to {quantity}")

    async def fetch_orders(
        self, credentials: dict[str, str], since: Optional[datetime] = None
    ) -> list[RemoteOrder]:
        _ = credentials, since
        return orders_for(self.code, "shy")
    ...
    async def push_listing(self, credentials: dict[str, str], listing: dict[str, Any]) -> PushResult:
        _ = credentials
        return PushResult(ok=True, external_id=f"shy-{listing.get('sku')}", message="Listing upserted on Shopify")

    async def fetch_payouts(
        self, credentials: dict[str, str], since: Optional[datetime] = None
    ) -> list[RemotePayout]:
        _ = credentials, since
        orders = orders_for(self.code, "shy")
        return payouts_for(self.code, "shy", orders)
